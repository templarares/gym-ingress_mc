...
def start_callback(action, name, controller):
    ...
    #     return config
        ...
        #left_hand.add("weight",int(2000*(abs(action[0]))))
        target=left_hand.add("target")
        target.add_array("rotation",np.concatenate([[0],action[1:4]*0.2])+[-0.114493,0.868807,0.412125,0.249437])
...
class IngressEnvRewards(gym.Env):
    ...
    def step(self, action):
        ...
        #     reward +=100*math.exp(-0.2*LH_couple[0])
        #     done=True
